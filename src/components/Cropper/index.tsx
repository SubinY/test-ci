...
> {
  ...
  constructor(props: ICropperProps) {
    ...
    this.state = {
      ...
      imgSrc: props.imgSrc,
      needHightLight: props.needHightLight || false,
      tempImgSrc: "",
  ...
  public fnInit() {
    ...
    Taro.createSelectorQuery()
      ...
      .context(res => {
        this._canvasContext =
          res?.context || Taro.createCanvasContext("cropCanvas");
  ...
  public fnTouchMove(e) {
    ...
    if (targetId === "box") {
      ...
      if (
        this._touchStartBoxLeft + this._touchStartBoxWidth - newBoxLeft <
        this._minBoxWidth
      ) {
        newBoxLeft =
          this._touchStartBoxLeft +
          this._touchStartBoxWidth -
          this._minBoxWidth;
      }
      if (
        this._touchStartBoxTop + this._touchStartBoxHeight - newBoxTop <
        this._minBoxHeight
      ) {
        newBoxTop =
          this._touchStartBoxTop +
          this._touchStartBoxHeight -
          this._minBoxHeight;
      }

      let newBoxWidth =
        this._touchStartBoxWidth - (newBoxLeft - this._touchStartBoxLeft);
      let newBoxHeight =
        this._touchStartBoxHeight - (newBoxTop - this._touchStartBoxTop);
      ...
      if (aspectRatio) {
        if (newBoxTop === 0 && newBoxLeft !== 0) {
          ...
          newBoxLeft =
            this._touchStartBoxWidth - newBoxWidth + this._touchStartBoxLeft;
        }
        if (newBoxLeft === 0) {
          newBoxHeight = newBoxWidth / aspectRatio;
          newBoxTop =
            this._touchStartBoxHeight - newBoxHeight + this._touchStartBoxTop;
        }
  ...
  public fnCrop(opts) {
    ...
    const imagePath = _self.state.imgSrc;
    ...
    this._canvasContext.draw(true, function() {
      Taro.canvasToTempFilePath({
        ...
        quality: quality,
        success: res => {
          console.log(_self.state.imgSrc, res.tempFilePath);
          _self.setState({
  ...
  public render() {
    const { onCropCallback, needHightLight } = this.props;
    const {
      imgSrc,
      stageWidth,
      stageHeight,
    ...
    const borderWidth = 2;
    const lTop = {
      x: (boxLeft + borderWidth) / stageWidth,
      y: (boxTop + borderWidth) / stageHeight
    };
    const rTop = {
      x: (boxLeft + boxWidth - borderWidth) / stageWidth,
      y: (boxTop + borderWidth) / stageHeight
    };
    const lBottom = {
      x: (boxLeft + borderWidth) / stageWidth,
      y: (boxTop + boxHeight - borderWidth) / stageHeight
    };
    const rBottom = {
      x: (boxLeft + boxWidth - borderWidth) / stageWidth,
      y: (boxTop + boxHeight - borderWidth) / stageHeight
    };
    const imgStyle = `width:100vw;height:${100 /
      (stageWidth / stageHeight)}vw;`;
    const clipStyle = hideBox
      ? "opacity: 0.5;"
      : `clip-path: polygon(${lTop.x * 100}% ${lTop.y * 100}%, ${rTop.x *
          100}% ${rTop.y * 100}%, ${rBottom.x * 100}% ${rBottom.y *
          100}%, ${lBottom.x * 100}% ${lBottom.y * 100}%);`;
      ...
      >
        ...
        {needHightLight && (
          <Image
            ...
            className="image"
            src={imgSrc}
            style={`${clipStyle}${imgStyle}${imgStyle1}`}
            mode="widthFix"
          ></Image>
        )}
        <Image
        ...
        <View
          className="stage"
          id="stage"
          style={`width:${stageWidth}px;height:${stageHeight}px;left:${stageLeft}px;top:${
            isHeightLarge ? 0 : stageTop
          }px;`}
          onTouchStart={this.fnTouchStart.bind(this)}
          onTouchMove={this.fnTouchMove.bind(this)}
          onTouchEnd={this.fnTouchEnd.bind(this)}
          onTouchCancel={this.fnTouchCancel.bind(this)}
        >
          <View
            id="box"
            className="box"
            style={`width:${boxWidth}px;height:${boxHeight}px;left:${boxLeft}px;top:${boxTop}px;${boxHideStyle}`}
          >
        ...
        <Canvas
          canvasId="cropCanvas"
          style={`height:${canvasHeight}px;width:${canvasWidth}px;position:fixed;background-color:red;left:5000rpx;`}
        ></Canvas>
      </ScrollView>
    );
  }
